    ...
    stages {
        ...
        stage('Build Docker Image ') {
            steps {
                sh '''#!/bin/bash -xe
	       		...
	       		docker build --build-arg FILE_NAME=${FILE_NAME} --build-arg FTP_PATH=${FTP_PATH} -f Dockerfile-Horovod-PyTorch -t yi/horovod:pytorch-python-${python_version} .
		            ''' 
            }
        }
	    stage('Test Docker Image') { 
		   ...
		   echo 'Hello, Tensorflow_Docker'
                    image_id="$(docker images -q yi/horovod:pytorch-python-${python_version})"
                      if [[ "$(docker images -q yi/horovod:pytorch-python-${python_version} 2> /dev/null)" == "$image_id" ]]; then
		...
		stage('Save & Load Docker Image') { 
            steps {
		        ...
		        echo 'Saving Docker image into tar archive'
                        docker save yi/horovod:pytorch-python-${python_version} | pv | cat > $WORKSPACE/yi-horovod-pytorch-python-${python_version}.tar
			
                        echo 'Remove Original Docker Images' 
			CURRENT_ID=$(docker images | grep -E '^yi/horovod.*'pytorch-python-${python_version}'' | awk -e '{print $3}')
			docker rmi -f yi/horovod:pytorch-python-${python_version}
			docker rmi -f nvidia/cuda:9.0-devel-ubuntu16.04
			
                        echo 'Loading Docker Image'
                        pv -f $WORKSPACE/yi-horovod-pytorch-python-${python_version}.tar | docker load
			docker tag $CURRENT_ID yi/horovod:pytorch-python-${python_version}
                        
                        echo 'Removing temp archive.'  
                        rm $WORKSPACE/yi-horovod-pytorch-python-${python_version}.tar
                   ''' 
		    }
		}
    }
