  ...
  agent {label 'yi-tflow-vnc'}
    stages {
        stage('Import Cuda Basic Docker Image') {
            steps {
                sh '''#!/bin/bash -xe
                   # Bacic Docker Image For Tensorflow Version 1.13 & Horovod with Cuda 10
                      image_id="$(docker images -q nvidia/cuda:10.0-devel-ubuntu18.04)"
                      echo "Basic Docker Image For Current Branch Is: $image_id"
                   
                      # Check If Docker Image Exist On Desired Server
                      if [[ "$(docker images -q nvidia/cuda:10.0-devel-ubuntu18.04 2> /dev/null)" == "" ]]; then
            ...
            steps {
                sh '''#!/bin/bash -xe
                      ...
                      echo 'Remove Original Docker Images' 
			                ...
			                docker rmi -f nvidia/cuda:10.0-devel-ubuntu18.04
