    ...
    stages {
        ...
        stage('Build Docker Image ') {
            steps {
                sh '''#!/bin/bash -xe
	       		...
	       		docker build --build-arg FILE_NAME=${FILE_NAME} --build-arg FTP_PATH=${FTP_PATH} -f Dockerfile-tf-2.242-${python_version} -t yi/tflow-vnc:${tensorflow_version}-python-${python_version}-openvino .
		            ''' 
            }
        }
	    stage('Test Docker Image') { 
		   ...
		   echo 'Hello, Tensorflow_Docker'
                    image_id="$(docker images -q yi/tflow-vnc:${tensorflow_version}-python-${python_version}-openvino)"
                      if [[ "$(docker images -q yi/tflow-vnc:${tensorflow_version}-python-${python_version}-openvino 2> /dev/null)" == "$image_id" ]]; then
		...
		stage('Save & Load Docker Image') { 
            steps {
		        ...
		        echo 'Saving Docker image into tar archive'
                        docker save yi/tflow-vnc:${tensorflow_version}-python-${python_version}-openvino | pv | cat > $WORKSPACE/yi-tflow-vnc-${tensorflow_version}-python-${python_version}-openvino.tar
			
                        echo 'Remove Original Docker Image' 
			CURRENT_ID=$(docker images | grep -E '^yi/tflow-vnc.*'${tensorflow_version}-python-${python_version}-openvino'' | awk -e '{print $3}')
			docker rmi -f yi/tflow-vnc:${tensorflow_version}-python-${python_version}-openvino
			
                        echo 'Loading Docker Image'
                        pv -f $WORKSPACE/yi-tflow-vnc-${tensorflow_version}-python-${python_version}-openvino.tar | docker load
			docker tag $CURRENT_ID yi/tflow-vnc:${tensorflow_version}-python-${python_version}-openvino 
                        
                        echo 'Removing temp archive.'  
                        rm $WORKSPACE/yi-tflow-vnc-${tensorflow_version}-python-${python_version}-openvino.tar
                   ''' 
		    }
		}
    }
