  ...
  agent {label 'tflow-gpu-3.6'}
    stages {
        stage('Create Docker-Build Image For Tensorflow-GPU-MKL') {
            steps {
	              sh 'docker build -t yi/tflow-vnc:1.4.1-python-3.6.3 .'  
            }
        }
	      stage('Test Docker-Build Image') { 
            steps {
                sh '''#!/bin/bash -xe
		              echo 'Hello, YI-TFLOW!!'
                    image_id="$(docker images -q yi/tflow-vnc:1.4.1-python-3.6.3)"
                      if [[ "$(docker images -q yi/tflow-vnc:1.4.1-python-3.6.3 2> /dev/null)" == "$image_id" ]]; then
