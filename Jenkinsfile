        ...
        stage('Build Docker Image ') {
            steps {
                sh '''#!/bin/bash -xe
                        ...
                        FTP_PATH=$(awk -F [,] -v srv="$SRV" '$6==srv' map.csv |  awk -F [,] -v python_version="$python_version" '$5==python_version' | awk -F [,] -v tf_version="$tensorflow_version" '$7~tf_version'|  awk -F [,] -v cuda_version="$cuda_version" '$2==cuda_version'  | awk -F, '{print $4}')
